  ...
  if (!class_exists('wp_iv_directories')) {  	
			...
			private static $instance;			
				...
				public function __construct() {
					...
					add_action('wp_ajax_iv_directories_update_profile_setting', array(&$this, 'iv_directories_update_profile_setting')); //update personal info(private profile)
									
					
					add_action('wp_ajax_iv_directories_save_cpt1', array(&$this, 'iv_directories_save_cpt1')); //add new La Firm(private profile)
					add_action('wp_ajax_iv_directories_save_cpt2', array(&$this, 'iv_directories_save_cpt2'));	//add new Lawyer(private profile)			
					
					add_action('wp_ajax_iv_directories_update_cpt1', array(&$this, 'iv_directories_update_cpt1')); //edit Law Firm(private profile)								
					add_action('wp_ajax_iv_directories_update_cpt2', array(&$this, 'iv_directories_update_cpt2')); //edit Lawyer(private profile)
			...
			public function iv_directories_save_rating(){
            	...
            	if(!empty($total_count)) {
                	...
                	if(metadata_exists('post', $profile_user_id, $rating_text.'_rating')) {
                    ...
                    if(metadata_exists('post', $profile_user_id, $rating_text.'_count')) {
                        $count_the_rating=get_post_meta($profile_user_id,$rating_text.'_count',true);
                	    $count_the_rating = $count_the_rating[0];
                	} else {
                	    $count_the_rating = 0;
                	}
			...
			public function iv_directories_update_cpt1(){
					...
					if(empty($form_data['title'])) {
                        echo json_encode(array("code" => "error","msg"=>"Enter Law Firm Name"));
                        exit(0);
                    }
                    // elseif(empty($form_data['address']) || ($form_data['address'] == 'Cannot determine address at this location.')) {
                    //     echo json_encode(array("code" => "error","msg"=>"Enter An Address Manually"));
                    //     exit(0);
                    // }
                    elseif(empty($form_data['phone'])) {
                        echo json_encode(array("code" => "error","msg"=>"Enter A Phone Number (Under Contact Info Section)"));
                        exit(0);
                    }
                    elseif(empty($form_data['contact-email'])) {
                        echo json_encode(array("code" => "error","msg"=>"Enter An Email Address (Under Contact Info Section)"));
                        exit(0);
                    }
					...
					//post status change
					/*$select_query = "SELECT post_status FROM {$wpdb->prefix}posts WHERE id='" . $form_data['user_post_id']. "'";
                    $select_query = $wpdb->get_results($select_query) or die(mysql_error());
                    $current_post_status = $select_query['post_status'];
			...
			public function get_unique_keyword_values( $post_type, $key = 'keyword' ){
			    ...
			    public function iv_directories_update_cpt2(){
					...
					if(is_array($form_data['postcats']) && empty($form_data['postcats'])) {
					    echo json_encode(array("code" => "error","msg"=>"Choose a Legal Issue"));
					    exit(0);
					}elseif(is_array($form_data['postlocation']) && empty($form_data['postlocation'])) {
					    echo json_encode(array("code" => "error","msg"=>"Choose a location"));
					    exit(0);
					}
					elseif(empty($form_data['address']) || ($form_data['address'] == 'Cannot determine address at this location.')) {
					    echo json_encode(array("code" => "error","msg"=>"Enter An Address Manually"));
					...
					if(isset($form_data['postlocation'] )){ 
							
						wp_set_object_terms( $newpost_id, $form_data['postlocation'], $directory_url_2.'-location');
					}
					...
					// For Cost & Availability
					$availability_all='';
					if(isset($form_data['availability_arr'] )){
						$availability_name= $form_data['availability_arr'] ;							
						$i=0;$availability_all='';
						foreach($availability_name  as $one_availability){							
							$availability_all= $availability_all.",".sanitize_text_field($one_availability);
						}
						update_post_meta($newpost_id, 'availability', $availability_all);  	
					...
					$j=0;
					for($i=0;$i<20;$i++){
						delete_post_meta($newpost_id, '_office_location_'.$j); 							
						delete_post_meta($newpost_id, '_office_address_'.$j);							
						delete_post_meta($newpost_id, '_office_ph_'.$j);						
					}		
					// Delete End
					
				
					if(isset($form_data['office_location'] )) {
						$office_location= $form_data['office_location'];
						$office_address= $form_data['office_address'];	
						$office_phone= $form_data['office_ph'];								
													
						$j=0;							
						for($j=0;$j<20;$j++){		
							if(isset($office_location[$j])){
								update_post_meta($newpost_id, '_office_location_'.$j, sanitize_text_field($office_location[$j])); 
							}
							if(isset($office_address[$j])){
								update_post_meta($newpost_id, '_office_address_'.$j, sanitize_text_field($office_address[$j])); 
							}
							if(isset($office_phone[$j])){
								update_post_meta($newpost_id, '_office_ph_'.$j, sanitize_text_field($office_phone[$j])); 
							}
				...
				public function iv_directories_save_cpt2(){
					...
					//print_r($form_data['postcats']);
					//print_r($form_data['postlocation']);
					//die('OKAY');
					
					if(empty($form_data['title'])) {
					    echo json_encode(array("code" => "error","msg"=>"Enter Lawyers Full name"));
                        exit(0);
					}elseif(empty($form_data['postcats'])) {
					    echo json_encode(array("code" => "error","msg"=>"Choose a Legal Issue"));
                        exit(0);
					}
					elseif(empty($form_data['postlocation'])) {
                        echo json_encode(array("code" => "error","msg"=>"Choose a location"));
                        exit(0);
                    }
                    elseif(empty($form_data['address']) || ($form_data['address'] == 'Cannot determine address at this location.')) {
					...
					if(isset($form_data['postcats'] )){ //Legal Issues
				        $category_ids = $form_data['postcats'];
                        wp_set_object_terms( $newpost_id, $category_ids, $directory_url_2.'-category');
					}
					if(isset($form_data['postlocation'] )){
					...
					if(isset($form_data['office_location'] )){
						$office_location= $form_data['office_location'];
						$office_address= $form_data['office_address'];	
						$office_ph= $form_data['office_ph'];							
													
						$j=0;
						//foreach($award_title  as $one_award_title){							
						for($j=0;$j<20;$j++){		
							if(isset($office_location[$i])){
								update_post_meta($newpost_id, '_office_location_'.$j, sanitize_text_field($office_location[$j])); 
							}
							if(isset($office_address[$i])){
								update_post_meta($newpost_id, '_office_address_'.$j, sanitize_text_field($office_address[$j])); 
							}
							if(isset($office_ph[$i])){
								update_post_meta($newpost_id, '_office_ph_'.$j, sanitize_text_field($office_ph[$j])); 
							}
				...
				public function iv_directories_update_profile_setting(){
					...
					if(empty($form_data['first_name'])) {
                        echo json_encode(array("code" => "error","msg"=>"Enter First name"));
                        exit(0);
                    }elseif(empty($form_data['last_name'])) {
                        echo json_encode(array("code" => "error","msg"=>"Choose a Last Name"));
                        exit(0);
                    }
				...
				*/
				...
				public function iv_directories_check_package_amount(){
					...
					if(!empty($post_cont) && sizeof($post_cont)>0){
						 $coupon_name = $post_cont->post_title;
